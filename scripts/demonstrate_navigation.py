...
if __name__ == '__main__':
    ...
    with sm:
        ...
        smach.StateMachine.add('findGates',
            searchFor(targets=['red buoy','red buoy','green buoy','green buoy']),
        ...
        def midpointStart(userdata, goal):
            ...
            for i in userdata.objects:
                listener.waitForTransform('/'+i.frame_id, '/base_link', rospy.Time(0),rospy.Duration(4.0)) # im assuming this is the relevant thing?
                (trans,rot) =listener.lookupTransform('/'+i.frame_id, '/base_link', rospy.Time(0))
            ...
            mba.target_pose.pose.position.x = midpoint[0]
            mba.target_pose.pose.position.y = midpoint[1]
